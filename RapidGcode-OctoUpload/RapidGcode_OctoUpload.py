...
if "OCTO_IP" not in locals():
    ...
    if os.path.exists("OctoIP.txt"):
        ...
        with open("OctoIP.txt", "r") as ip_file:
            for line in ip_file:
                if not line.startswith('#'):
                    OCTO_IP = line.strip()
                    break
        print("*** Octo Upload is ON! - If you want to enter a new IP change the IP in OctoIP.txt or delete OctoIP.txt ***")
    else:
        ...
        if octo_setup.lower() == "y":
            ...
            OCTO_IP = input("Enter the desired OCTO IP: ")
            # Save the OCTO_IP to OctoIP.txt
            with open("OctoIP.txt", "w") as ip_file:
                ip_file.write("# Delete this file or change the IP in this file if you need to change IP.\n")
